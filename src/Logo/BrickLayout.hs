...
data BrickGeom = BrickGeom
    { bgBodyH :: !Int
    -- ^ Brick body height in SVG px (e.g., 17)
    , bgInnerBodyH :: !Int
    -- ^ Vertical row pitch / v_pitch (e.g., 15)
    , bgHPitch :: !Int
    -- ^ Horizontal column pitch / h_pitch (e.g., 12)
    }
...
data BrickLayout = BrickLayout
    { blBlkW :: !Int
    -- ^ Brick SVG unit width (e.g., 24 → hPitch = 12)
    , blBlkH :: !Int
    -- ^ Brick SVG unit height (e.g., 20)
    , blPadTop :: !Int
    -- ^ Extra SVG pixels above the brick grid
    , blPadBottom :: !Int
    -- ^ Extra SVG pixels below the brick grid
    , blPadLeft :: !Int
    -- ^ Extra SVG pixels left of the brick grid
    , blPadRight :: !Int
    -- ^ Extra SVG pixels right of the brick grid
    , blRows :: [BrickRow]
...
imageAndMapToLayout img brickMap blkW blkH padTop padBottom =
    let hPitch = blkW `div` 2
     in BrickLayout
            ...
            , blRows =
                map
                    (buildRow img brickMap hPitch (imageWidth img))
                    [0 .. imageHeight img - 1]
            }
...
brickSvgElements brickX brickY brickW color geom =
    let r = rgbStr color
        ...
        lTop =
            printf
                "  <line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\" stroke=\"%s\" stroke-width=\"0.25\" opacity=\"1.0\"/>"
                brickX
                bodyY
                (brickX + brickW)
                bodyY
...
mkStud brickX brickY r bc studHt hPitch i =
    let studW = 7 :: Int
        ...
        sRect =
            printf
                "  <rect x=\"%.1f\" y=\"%d\" width=\"%d\" height=\"%d\" fill=\"%s\"/>"
                studX
                studY
                studW
                studHt
        ...
        sBdr =
            printf
                "  <rect x=\"%.1f\" y=\"%d\" width=\"%d\" height=\"%d\" fill=\"none\" stroke=\"%s\" stroke-width=\"0.25\" opacity=\"1.0\"/>"
                studX
                studY
                studW
                studHt
