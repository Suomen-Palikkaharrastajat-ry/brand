...
data PadXMode
    = FixedPadX Int  -- ^ Fixed SVG px each side (use 0 for no extra padding)
    | AutoSquare     -- ^ Compute padding so that canvasW == canvasH
...
composeLogoWith
    ...
    -> Maybe Text   -- ^ Subtitle text line 2 (Nothing = single-line)
    -> Text         -- ^ Subtitle CSS colour (e.g. @\"#05131D\"@)
    -> Text         -- ^ Input brick SVG text
    -> Int          -- ^ Font size in SVG units
    -> Int          -- ^ Font weight (e.g. 400 or 700)
    -> PadXMode     -- ^ Horizontal padding mode
    -> Text
composeLogoWith fontDataUri subtitleText mSubtitleText2 subtitleColor srcText txtSize fontWeight padXMode =
    ...
    in buildFullSvg srcText canvasW canvasH brickH
                    txtSize fontWeight subtitleColor fontDataUri
                    subtitleText mSubtitleText2 lineGap translateX
...
buildFullSvg
    ...
    -> Text       -- ^ subtitle text line 1
    -> Maybe Text -- ^ subtitle text line 2 (Nothing = single-line)
    -> Int        -- ^ line gap for line 2 (ignored when Nothing)
    -> Double     -- ^ brick group translate-X (SVG units, may be fractional)
    -> Text
buildFullSvg srcText canvasW canvasH brickH txtSize fontWeight subtitleColor fontDataUri subtitleText mSubtitleText2 lineGap translateX =
  ...
  where
    ...
    mkTextElem y txt =
        "<text"
            <> " x=\"" <> showI cx <> "\""
            <> " y=\"" <> T.pack (show (fromIntegral y :: Double)) <> "\""
            <> sharedAttrs
            <> ">"
            <> txt
            <> "</text>"
